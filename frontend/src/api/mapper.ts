...
export function toRecommendation(item: RecommendationItem, run: RecommendationRun): Recommendation {
  ...
  return {
    ...
    status: item.recommended_quantity === null ? "blocked" : item.recommended_quantity > 0 ? "recommended" : "no_order",
    recommended_qty: item.recommended_quantity,
    preview_qty: item.calculated_quantity,
    urgency: item.urgency,
    explanation: item.explanation,
    available_stock: c.stock_estimate?.quantity ?? c.available_stock,
    ...
    breakdown: {
      ...
      stock_snapshot_date: c.stock_estimate?.date ?? c.available_stock_date,
    ...
    data_warnings: item.flags.map((flag) => ({
      code: flag, message: flag === "estimated_stock" ? "Остаток оценён по началу месяца и продажам; это предварительный план." : flag === "forecast_only" ? "Прогноз спроса без количества заказа: остаток неизвестен." : flag, severity: "warning" as const,
...
export function runOrders(run: RecommendationRun): PurchaseOrder[] {
  return run.suppliers.map((group) => ({
    ...
    approved_at: null,
    lines: group.items.filter((item) => (item.calculated_quantity ?? 0) > 0 || (item.recommended_quantity ?? 0) > 0).map((item) => ({
      id: item.item_id,
